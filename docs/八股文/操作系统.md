...
- 相关接口
  ...
  **信号**
  ...
  ![在这里插入图片描述](https://gcore.jsdelivr.net/gh/CARLOSGP2021/myFigures/img/202206291607953.png)
...
**信号安全**：在信号处理中，发生信号异常时调用信号处理函数，都不会造成除了信号处理函数以外的其他功能变化。可重入函数就满足了信号安全的特征，所以通常不可重入函数不是信号安全的，就像malloc/free内部有维护全局的共享内存链表，且加锁了，是不可重入函数，所以malloc/free是信号非安全函数。
...
对于 Linux 来说，通过区分内核空间和用户空间的设计，隔离了操作系统代码(操作系统的代码要比应用程序的代码健壮很多)与应用程序代码。即便是单个应用程序出现错误也不会影响到操作系统的稳定性，这样其它的程序还可以正常的运行(Linux 可是个多任务系啊！)。
...
[用户态和内核态切换_燕然一勒的博客-CSDN博客_用户态和内核态的切换](https://blog.csdn.net/qq_42052956/article/details/111562280)
...
[缺页中断过程详解_interval_package的博客-CSDN博客_缺页中断的实现过程](https://blog.csdn.net/interval_package/article/details/124423355)
